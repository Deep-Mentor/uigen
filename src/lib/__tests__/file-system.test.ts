...
describe("getNode", () => {
  test("returns the correct node for a file", () => {
    const fs = new VirtualFileSystem();
    fs.createFile("/hello.ts", "export {}");

    const node = fs.getNode("/hello.ts");
    expect(node).not.toBeNull();
    expect(node?.type).toBe("file");
    expect(node?.name).toBe("hello.ts");
    expect(node?.path).toBe("/hello.ts");
    expect(node?.content).toBe("export {}");
  });

  test("returns the correct node for a directory", () => {
    const fs = new VirtualFileSystem();
    fs.createDirectory("/src");
  ...
  test("normalizes the path before lookup", () => {
    const fs = new VirtualFileSystem();
    fs.createFile("/test.ts", "content");

    expect(fs.getNode("test.ts")).not.toBeNull();
    expect(fs.getNode("//test.ts")).not.toBeNull();
  });
});
...
describe("createDirectory", () => {
  test("returns null when parent directory does not exist", () => {
    const fs = new VirtualFileSystem();
    const result = fs.createDirectory("/missing-parent/child");

    expect(result).toBeNull();
    expect(fs.exists("/missing-parent/child")).toBe(false);
  });
});

describe("serialize", () => {
  test("directory nodes omit children and content fields", () => {
    const fs = new VirtualFileSystem();
    fs.createDirectory("/src");
    fs.createFile("/src/index.ts", "export {}");
    ...
    expect(Object.keys(serialized["/src"])).not.toContain("children");
    expect(Object.keys(serialized["/src"])).not.toContain("content");
  });

  test("file nodes include content even when empty", () => {
    const fs = new VirtualFileSystem();
    fs.createFile("/empty.ts", "");

    const serialized = fs.serialize();

    expect(serialized["/empty.ts"].type).toBe("file");
    expect(serialized["/empty.ts"].content).toBe("");
  });

  test("returns an empty filesystem with only root when no files exist", () => {
    const fs = new VirtualFileSystem();
    const serialized = fs.serialize();
...
describe("deserialize clears existing state", () => {
  test("removes files that existed before deserialization", () => {
    const fs = new VirtualFileSystem();
    fs.createFile("/old.txt", "stale");

    fs.deserialize({ "/new.txt": "fresh" });

    expect(fs.exists("/old.txt")).toBe(false);
    expect(fs.readFile("/new.txt")).toBe("fresh");
  });
});

describe("deserializeFromNodes clears existing state", () => {
  test("removes files that existed before deserialization", () => {
    const fs = new VirtualFileSystem();
    fs.createFile("/old.txt", "stale");
    ...
    fs.deserializeFromNodes({
      ...
      "/new.ts": { type: "file", name: "new.ts", path: "/new.ts", content: "fresh" },
    });

    expect(fs.exists("/old.txt")).toBe(false);
    expect(fs.readFile("/new.ts")).toBe("fresh");
  });

  test("skips the root entry without creating a duplicate", () => {
    const fs = new VirtualFileSystem();

    fs.deserializeFromNodes({
      "/": { type: "directory", name: "/", path: "/" },
      "/a.ts": { type: "file", name: "a.ts", path: "/a.ts", content: "a" },
    });

    expect(fs.getNode("/")).not.toBeNull();
    expect(fs.readFile("/a.ts")).toBe("a");
  });
});

describe("viewFile range edge cases", () => {
  test("clamps start below 1 up to line 1", () => {
    const fs = new VirtualFileSystem();
    fs.createFile("/f.txt", "line1\nline2\nline3");

    const view = fs.viewFile("/f.txt", [0, 2]);
    expect(view).toBe("1\tline1\n2\tline2");
  });

  test("clamps end beyond file length to the last line", () => {
    const fs = new VirtualFileSystem();
    fs.createFile("/f.txt", "line1\nline2");

    const view = fs.viewFile("/f.txt", [1, 100]);
    expect(view).toBe("1\tline1\n2\tline2");
  });
});

describe("replaceInFile with regex special characters", () => {
  test("treats the search string as a literal, not a regex", () => {
    const fs = new VirtualFileSystem();
    fs.createFile("/f.ts", "price: $10.00 and $20.00");

    const result = fs.replaceInFile("/f.ts", "$10.00", "$9.99");

    expect(result).toBe("Replaced 1 occurrence(s) of the string in /f.ts");
    expect(fs.readFile("/f.ts")).toBe("price: $9.99 and $20.00");
  });

  test("handles parentheses and brackets in the search string", () => {
    const fs = new VirtualFileSystem();
    fs.createFile("/f.ts", "fn(a[0])");

    const result = fs.replaceInFile("/f.ts", "fn(a[0])", "fn(b[1])");

    expect(result).toBe("Replaced 1 occurrence(s) of the string in /f.ts");
    expect(fs.readFile("/f.ts")).toBe("fn(b[1])");
  });
});

describe("reset", () => {
  test("removes all files and directories", () => {
    const fs = new VirtualFileSystem();
    fs.createDirectory("/src");
    fs.createFile("/src/index.ts", "content");
  ...
  test("root directory is still accessible after reset", () => {
    const fs = new VirtualFileSystem();
    fs.createFile("/test.ts", "content");
  ...
  test("filesystem is fully usable after reset", () => {
    const fs = new VirtualFileSystem();
    fs.createFile("/before.ts", "old");

    fs.reset();
    fs.createFile("/after.ts", "new");

    expect(fs.readFile("/after.ts")).toBe("new");
    expect(fs.getAllFiles().size).toBe(1);
  });
});
